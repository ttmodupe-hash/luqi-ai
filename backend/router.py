...
Restored 2026-09-18 from commit 4628fb19 (2026-07-24) — the file had been
clobbered to a 13-byte PASTE_CONTENT placeholder (direct no-stubs guardrail
violation). A real FastAPI surface was appended so the main.py mount at
prefix="/api" actually serves this module.
"""
...
def classify_intent(query: str) -> Dict[str, Any]:
    ...
    for intent_id, intent_data in INTENT_MAP.items():
        ...
        if score > 0:
            scores[intent_id] = {"score": score, "handler": intent_data["handler"], "priority": intent_data["priority"], "matched_keywords": matched_keywords}
    if not scores:
        return {"intent": "general", "handler": "luqi_agent", "confidence": 0, "matched_keywords": []}
    best = max(scores.items(), key=lambda x: (x[1]["score"], -x[1]["priority"]))
    return {"intent": best[0], "handler": best[1]["handler"], "confidence": best[1]["score"], "matched_keywords": best[1]["matched_keywords"], "all_matches": {k: {"score": v["score"], "keywords": v["matched_keywords"]} for k, v in scores.items()}}
...
def route(query: str, context: Dict = None) -> Dict[str, Any]:
    ...
    result = {"status": "routed", "query": query, "intent": classification["intent"], "handler": handler_name, "confidence": classification["confidence"], "matched_keywords": classification["matched_keywords"], "handler_loaded": handler is not None}
    if handler:
        try:
...
def get_intents() -> Dict[str, Any]:
    return {"status": "success", "total_intents": len(INTENT_MAP), "intents": [{"id": k, "handler": v["handler"], "keyword_count": len(v["keywords"])} for k, v in INTENT_MAP.items()]}
...
def get_handler_info(handler_name: str) -> Dict[str, Any]:
    ...
    return {"status": "success", "handler": handler_name, "module": handler.__name__ if hasattr(handler, '__name__') else str(handler), "available_functions": functions}
...
def health_check() -> Dict[str, Any]:
    ...
    return {"status": "healthy" if all_loaded else "degraded", "router": "active", "handlers": handlers, "total_handlers": len(handlers), "loaded": sum(1 for v in handlers.values() if v == "loaded")}


# ── FastAPI surface (mounted by main.py at prefix="/api") ─────────────────
# Thin, real wrapper around the verified functions above — no new logic.
...
@router.post("/route")
def route_query(req: RouteRequest) -> Dict[str, Any]:
    """Classify intent and dispatch a query to the matching backend handler."""
    return route(req.query, req.context)


@router.post("/route/classify")
def classify_only(req: RouteRequest) -> Dict[str, Any]:
    """Return the intent classification without invoking a handler."""
    return classify_intent(req.query)


@router.get("/intents")
def list_intents() -> Dict[str, Any]:
    """List all registered intents and their handlers."""
    return get_intents()


@router.get("/router/health")
def router_health() -> Dict[str, Any]:
    """Router health: which handler modules actually loaded."""
    return health_check()


@router.get("/router/handler/{handler_name}")
def handler_info(handler_name: str) -> Dict[str, Any]:
    """Inspect a handler module's available functions."""
    return get_handler_info(handler_name)
