...
class SelfImprovementLab:
    ...
    def benchmark_capabilities(self) -> dict[str, Any]:
        ...
        Actually instantiates each capability class, calls a representative
        method, validates the result, and records timing + pass/fail status.
        """
        tests: dict[str, dict] = {
            "deep_research": {
            ...
            "investment": {
                ...
                "check": lambda r: isinstance(r, dict) and "daily" in r and isinstance(r.get("daily"), dict) and "profit_usd" in r["daily"],
            },
            "tax": {
        ...
        # Module-name -> (class_name, module_name) mapping for dynamic imports
        ...
        for cap_name, test_cfg in tests.items():
            ...
            try:
                ...
                # Build result preview (first 100 chars)
                if isinstance(result, str):
                    result_preview = result[:100]
                elif isinstance(result, (dict, list)):
                    preview_str = json.dumps(result)
                    result_preview = preview_str[:100]
                else:
                ...
                error_msg = f"Missing method/attribute: {exc}"
                failed_count += 1
        ...
        # Cache for lab_report() consumption
    ...
    def lab_report(self) -> str:
        ...
        if self._last_benchmark is not None:
            ...
            lines.append(f"  Passed: {bench['passed']}/{bench['total']}  "
                         f"Failed: {bench['failed']}  "
                         f"Total time: {bench['total_time_ms']} ms")
...
if __name__ == "__main__":
    ...
    print(f"Total time: {bench['total_time_ms']} ms")
    print(f"Timestamp: {bench['timestamp']}")
    for cap, detail in bench["details"].items():
        icon = "OK" if detail["status"] == "PASS" else "XX"
        print(f"  [{icon}] {cap}: {detail['response_time_ms']} ms  "
              f"err={detail['error']}  preview={detail['result_preview'][:50]}")
